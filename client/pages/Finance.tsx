...
const StatCard = ({
  ...
  return (
    <Card className={`p-6 ${getVariantStyles()} shadow-sm transition-colors duration-300`}>
...
const Finance: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'on-track':
        return <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20">On Track</Badge>;
      case 'over-budget':
        return <Badge variant="destructive" className="bg-red-500/20 text-red-600 dark:text-red-400 border-red-500/20">Over Budget</Badge>;
      case 'under-budget':
        return <Badge variant="outline" className="border-border">Under Budget</Badge>;
      default:
  ...
  return (
    <div className="space-y-6 p-6 bg-background min-h-screen transition-colors duration-300">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground">Financial Overview</h1>
          <p className="text-muted-foreground mt-1">Monitor church financial health and stewardship</p>
        </div>
              ...
              <div key={dept.department} className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 bg-muted rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${dept.status === 'over-budget' ? 'bg-red-500' : 'bg-primary'
                        }`}
                      style={{
              ...
              <div key={transaction.id} className="flex items-center justify-between p-3 hover:bg-muted/50 rounded-lg transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${transaction.type === 'income'
                      ...
                      : 'bg-red-100/20 text-red-600 dark:text-red-400'
                    }`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{transaction.date} • {transaction.department}</p>
                  </div>
                </div>
            ...
            <TableBody>
              {quarterlyBudget.map((quarter) => (
                <TableRow key={quarter.quarter}>
                  ...
                  <TableCell className={`text-right font-medium ${quarter.variance >= 0 ? 'text-primary' : 'text-red-600 dark:text-red-400'
                    }`}>
                  ...
                  <TableCell className="text-center">
                    {quarter.variance >= 0 ? (
                      ...
                      <Badge variant="secondary" className="bg-red-100/20 text-red-800 dark:text-red-400 border-red-500/20">
