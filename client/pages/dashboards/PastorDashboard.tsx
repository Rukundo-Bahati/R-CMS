...
const StatCard = ({
    ...
    const bg = variant === 'primary' ? 'bg-primary text-primary-foreground' : 'bg-card text-card-foreground';
    const border = variant === 'primary' ? 'border-primary' : 'border border-border';
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-foreground">Pastor Dashboard</h1>
                    <p className="text-muted-foreground mt-1">Manage preachers, sermons, and pastoral activities</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="p-6">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold text-foreground">Weekly Preaching Schedule</h3>
                    </div>
                    <div className="h-80">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-semibold text-foreground">Upcoming Sermons</h3>
                        <Button
                            variant="outline"
                            size="sm"
                    ...
                    <div className="space-y-4">
                        {upcomingSermonsData.map((sermon) => (
                            <div key={sermon.id} className="flex items-start gap-3 p-3 hover:bg-accent rounded-lg transition-colors border border-transparent hover:border-border">
                                <div className="bg-primary/10 p-2 rounded-lg">
                                    <Mic className="w-5 h-5 text-primary" />
                                </div>
                                <div className="flex-1">
                                    <p className="font-medium text-foreground">{sermon.title}</p>
                                    <p className="text-sm text-muted-foreground">{sermon.date} • {sermon.time}</p>
                                    <p className="text-xs text-muted-foreground/60 mt-1">Preacher: {sermon.preacher}</p>
                                    <p className="text-xs text-indigo-500 dark:text-indigo-400 font-medium mt-1">Topic: {sermon.topic}</p>
                                </div>
                            </div>
                        ))}
                    </div>
