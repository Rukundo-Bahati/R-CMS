...
import { Users, User, Calendar, Clock, AlertCircle, TrendingUp, TrendingDown, Loader2 } from "lucide-react";
...
const getFamilyMemberCounts = (families: Family[]) => {
...
const getFormattedFamilyData = (familyData: any[], totalMembers: number) => {
  return familyData.map(family => ({
    ...family,
    percentage: totalMembers > 0 ? Math.round((family.value / totalMembers) * 100) : 0,
    displayName: `${family.name} (${family.generation})`
  }));
};

// Prepare family data for the bar chart
const getFamilyGrowthData = (formattedData: any[]) => {
  return formattedData.map(family => ({
    name: family.name,
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className={`text-sm font-medium ${variant === 'primary' ? 'text-primary-foreground/90' : 'text-muted-foreground'}`}>
              {title}
            </p>
          </div>
...
export default function GrandPereMereDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardStats = async () => {
      setLoading(true);
      try {
        const data = await api.grandparents.getStats(timeRange);
  ...
  const { stats, recentActivities: dbActivities, familyDistribution: dbDistribution } = dashboardData;

  // Map activities for display
  const mappedActivities = dbActivities.map((act: any, idx: number) => ({
    id: idx,
    title: act.content,
  ...
  const mappedUpcomingEvents = dashboardData.upcomingEvents || [];

  // Map distribution for charts
  const mappedDistribution = dbDistribution.map((fam: any) => ({
    name: fam.name,
    value: parseInt(fam.total),
    boys: parseInt(fam.boys),
    girls: parseInt(fam.girls),
  }));

  const totalFamilies = stats.totalFamilies;
  const totalMembers = stats.totalMembers;
  const totalMale = stats.totalBoys;
  const totalFemale = stats.totalGirls;
  const totalY1 = stats.y1;
  const totalY2 = stats.y2;
  const totalY3 = stats.y3;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Welcome back! Here's what's happening with your community.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-lg font-semibold text-foreground">Families Distribution</h3>
              <p className="text-sm text-muted-foreground">Member count by family</p>
            </div>
            <Button variant="ghost" size="sm" className="text-primary hover:bg-primary/5" asChild>
              <Link to="/dashboard/grand_pere_mere/families">View All</Link>
            </Button>
          </div>
          <div className="h-96">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mappedDistribution}>
        ...
        <Card className="p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
            <div>
              <h3 className="text-lg font-semibold text-foreground">Members Growth</h3>
              <p className="text-sm text-muted-foreground">Monthly member count by family</p>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card className="p-6 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="space-y-4">
            {mappedActivities.length > 0 ? mappedActivities.map((activity: any) => (
              <ActivityItem
                key={activity.id}
        ...
        <Card className="p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Scheduled activities and meetings</p>
            </div>
            <Button variant="ghost" size="sm" className="text-primary hover:bg-primary/5" asChild>
              <Link to="/dashboard/grand_pere_mere/schedule">View All</Link>
            </Button>
          </div>
          <div className="space-y-4">
            {mappedUpcomingEvents.length > 0 ? mappedUpcomingEvents.map((event: any) => (
              <div
                key={event.id}
                className="flex items-center p-4 bg-card rounded-lg border border-border hover:border-primary/20 hover:shadow-sm transition-all"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center text-sm text-muted-foreground mt-1 flex-wrap gap-x-2">
                    ...
                    <div className="flex items-center">
                      <svg className="w-4 h-4 mr-1 text-muted-foreground/60" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                      </svg>
                      <span>{event.location || 'Church'}</span>
                    </div>
                  </div>
                </div>
              </div>
