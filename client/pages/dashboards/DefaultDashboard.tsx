        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={member.name} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                <div>
                  <p className="font-medium text-foreground">{member.name}</p>
                  <p className="text-sm text-muted-foreground">{member.dept}</p>
                </div>
                <p className="text-sm text-muted-foreground/60">{member.date}</p>
              </div>
            ))}
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={event.title} className="flex items-center justify-between py-3 border-b border-border">
                <div>
                  <p className="font-medium text-foreground">{event.title}</p>
                  <p className="text-sm text-muted-foreground">{event.date}</p>
                </div>
                <span className={`text-xs font-semibold px-3 py-1 rounded-full ${event.status === "Confirmed"
                  ? "bg-green-100 text-green-700 dark:bg-green-900/10 dark:text-green-400"
                  : event.status === "Scheduled"
                    ? "bg-blue-100 text-blue-700 dark:bg-blue-900/10 dark:text-blue-400"
                    : "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/10 dark:text-yellow-400"
                  }`}>
                  {event.status}
                </span>
              </div>
